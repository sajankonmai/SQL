...
This project utilizes the following PostgreSQL tables **'netflix_titles_info'** and **'netflix_people'** and performs some calculations.
...
#### PostgreSQL query to show movies/tv shows details.
```
SELECT title, netflix_titles_info.type, release_year, director, netflix_people.cast
...
#### PostgreSQL query to show movies title.
```
SELECT COUNT(*)
FROM "netflix_titles_info"
WHERE type = 'Movie';
...
#### PostgreSQL query to show the most recent batch of movies/tv shows added to the database.
```
SELECT MAX(date_added)
FROM "netflix_titles_info";
```

#### PostgreSQL query to show movies/tv shows in an alphabetical order.
```
SELECT title
FROM "netflix_titles_info"
ORDER BY title;
...
#### PostgreSQL query to find the director of the movie 'The Starling'.
```
SELECT director
FROM "netflix_people"
...
#### PostgreSQL query to show the oldest movie.
```
